...
> **本番反映は deploy CI 初回実行（v8）にて**（統括作業待ち。push・PR・マージ・Azure OIDC secrets 登録・GitHub Environment 保護設定が未了）。実戦知見は [[01_ナレッジ/共通/github-actions-dependency-vulnerability-ci-gate|GitHub Actions による依存脆弱性の CI ゲート（Snyk＋Dependabot）]] を参照。
