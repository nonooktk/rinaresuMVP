...
| Medium | 1 | F-4 依存の既知脆弱性（backend18・frontend3） | ⏳ 次段 |
...
4. ⏳ **［次段・任意］F-4**: backend の python-multipart・requests を更新、starlette は fastapi 追随。frontend は `npm audit fix`。更新後に回帰確認。
...
残る本丸だった F-1・F-2（＋F-3）は、認証を「検証可能な資格情報（署名付きセッション通行証）」に変え、掛け忘れた GET／PDF に認証＋本人スコープを足すことで**修正・本番デプロイ済み（v7）**。→ エンプラ最低ラインに到達。操作系の認可は元から正しく、土台の良さがそのまま活きた。**次段は F-4（依存更新）・F-5（多層防御）**。
